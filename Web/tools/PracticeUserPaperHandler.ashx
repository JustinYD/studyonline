<%@ WebHandler Language="C#" Class="PracticeUserPaperHandler" %>
...
public class PracticeUserPaperHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        context.Response.Write(PracticeUserPapeRequest());
        context.Response.End();
    }

    private string PracticeUserPapeRequest()
    {
        switch (Action)
        {
            case "kclist":
    ...
    private string GetKcList()
    {
        ...
        try
        {
            ...
            strWhere += "AND exam.ExamGuid IN (SELECT ExamGuid FROM T_PracticeExamUser WHERE UserGuid='" + userGuid + "')";
            var strOrder = " CreateTime desc";
            var tableName = " T_PracticeExams exam LEFT JOIN T_PracticeExamUser exUser ON exUser.ExamGuid=exam.ExamGuid AND exUser.UserGuid='" + userGuid + "'";
    ...
    private string GetDetail()
    {
        ...
        try
        {
            ...
            {
                var paperGuid = ExamDAL.DbHelperSQL.GetSingle("SELECT PaperGuid FROM T_PracticeExamPager WHERE ExamGuid=@ExamGuid", new SqlParameter("@ExamGuid", examGuid)) + "";
                if (!string.IsNullOrEmpty(paperGuid))
                {
                    ...
                    if (paperDs != null && paperDs.Tables.Count > 0)
                    {
                        ...
                        if (paperList != null && paperList.Count > 0)
                        {
                            ...
                            var qDs = ExamDAL.DbHelperSQL.Query("SELECT Number,STKGuid,QuestionName,Video,QuestionScore,QuestionImg,QuestionType,QuestionGuid FROM T_PracticeQuestions WHERE QuestionGuid IN (SELECT QuestionGuid FROM T_PracticePaperQuestion WHERE PaperGuid=@PaperGuid) ", new SqlParameter("@PaperGuid", paperGuid));
                            if (qDs != null && qDs.Tables.Count > 0)
                            {
                                ...
                                if (paperModel.QuestionList != null && paperModel.QuestionList.Count > 0)
                                {
                                    ...
                                    var isAnswered = ExamDAL.DbHelperSQL.GetSingle("SELECT IsAnswered FROM T_PracticeExamUser WHERE ExamGuid=@ExamGuid AND UserGuid=@UserGuid", new SqlParameter("@ExamGuid", examGuid), new SqlParameter("@UserGuid", userGuid)) + "";
                                    paperModel.IsAnswered = int.Parse(isAnswered);
                                    if (paperModel.IsAnswered == 1) {
                                        paperModel.UserScore =double.Parse(ExamDAL.DbHelperSQL.GetSingle("SELECT UserScore FROM T_PracticeExamUser WHERE ExamGuid=@ExamGuid AND UserGuid=@UserGuid", new SqlParameter("@ExamGuid", examGuid), new SqlParameter("@UserGuid", userGuid)) + "");
                                    }
                                    for (int i = 0; i < paperModel.QuestionList.Count; i++)
                                    {
                                        ...
                                        {//已作答
                                            var selectStr = @"SELECT RightAnswerGuid,SelectAnswerGuid,IsRight FROM T_PracticeUserAnswer WHERE ExamGuid=@ExamGuid AND PaperGuid=@PaperGuid AND UserGuid=@UserGuid AND QuestionGuid=@QuestionGuid";
    ...
    private string SubmitPaper()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(json))
            {
                ...
                if (paperModel != null)
                {
                    var paperGuid = ExamDAL.DbHelperSQL.GetSingle("SELECT PaperGuid FROM T_PracticeExamPager WHERE ExamGuid=@ExamGuid", new SqlParameter("@ExamGuid", paperModel.ExamGuid)) + "";
                    ...
                    if (qDs != null && qDs.Tables.Count > 0)
                    {
                        ...
                        ExamDAL.DbHelperSQL.ExecuteSql("DELETE FROM T_PracticeUserAnswer WHERE ExamGuid=@ExamGuid AND PaperGuid=@PaperGuid AND UserGuid=@UserGuid", new SqlParameter("@ExamGuid", paperModel.ExamGuid), new SqlParameter("@PaperGuid", paperGuid), new SqlParameter("@UserGuid", paperModel.UserGuid));
                        if (questionList != null && questionList.Count > 0)
                        {
                            var insertStr = @"INSERT INTO [T_PracticeUserAnswer]
                                                       ([PaperAnswerGuid]
                                                       ,[ExamGuid]
                                                       ,[PaperGuid]
                                                       ,[UserGuid]
                    ...
                    var updateStr = @"UPDATE T_PracticeUserAnswer SET RightAnswerGuid=@RightAnswerGuid,SelectAnswerGuid=@SelectAnswerGuid,IsRight=@IsRight WHERE ExamGuid=@ExamGuid AND PaperGuid=@PaperGuid AND UserGuid=@UserGuid AND QuestionGuid=@QuestionGuid";
                    ...
                    var sumStr = "SELECT SUM(Score) FROM T_PracticeUserAnswer WHERE IsRight=1 AND ExamGuid=@ExamGuid AND PaperGuid=@PaperGuid AND UserGuid=@UserGuid";
                    ...
                    ExamDAL.DbHelperSQL.ExecuteSql("UPDATE T_PracticeExamUser SET IsAnswered=1,UserScore=@UserScore WHERE ExamGuid=@ExamGuid AND UserGuid=@UserGuid", new SqlParameter("@ExamGuid", paperModel.ExamGuid), new SqlParameter("@UserGuid", paperModel.UserGuid), new SqlParameter("@UserScore", (totalScore == null ? 0d : double.Parse(totalScore + ""))));
                    jsonResult.status = 1;
