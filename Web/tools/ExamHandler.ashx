<%@ WebHandler Language="C#" Class="ExamHandler" %>
...
public class ExamHandler : IHttpHandler
{
    ...
    private string KCOp()
    {
        ...
        try
        {
            ...
            var examBll = new ExamBLL.ExamsBLL();
            ...
            if (examModel != null)
            {
                ...
                {//编辑
                    ExamDAL.DbHelperSQL.ExecuteSql("DELETE FROM T_ExamUser WHERE ExamGuid=@ExamGuid", new SqlParameter("@ExamGuid", examModel.ExamGuid));
                    ExamDAL.DbHelperSQL.ExecuteSql("DELETE FROM T_ExamPager WHERE ExamGuid = @ExamGuid", new SqlParameter("@ExamGuid", examModel.ExamGuid));
                    tag = "EDITOR";
                }
                else
                {
                ...
                var examUserBll = new ExamBLL.ExamUserBLL();
                //新增人员和考次的关系
                ...
                //新增考次和试卷的关系
                var strSql = @"INSERT INTO [T_ExamPager] ([EQGuid] ,[ExamGuid] ,[PaperGuid] ,[Status] ,[CreateTime])VALUES (NEWID() ,@ExamGuid ,@PaperGuid ,1 ,GETDATE())";
    ...
    private string KcDetail()
    {
        ...
        try
        {
            ...
            {
                var strSql = "SELECT * FROM T_Exams WHERE ExamGuid=@ExamGuid";
                ...
                if (examModelDs != null && examModelDs.Tables.Count > 0 && examModelDs.Tables[0].Rows.Count > 0)
                {
                    ...
                    var paperDs = ExamDAL.DbHelperSQL.Query("SELECT * FROM T_Paper WHERE PaperGuid in (SELECT PaperGuid FROM T_ExamPager WHERE ExamGuid=@ExamGuid)", new SqlParameter("@ExamGuid", examGuid));
                    ...
                    var bll = new ExamBLL.UsersBLL();
                    examModel.UserList = bll.GetModelList(string.Format(" UserGuid IN (SELECT UserGuid FROM T_ExamUser WHERE ExamGuid='{0}' )", examGuid));
                    jsonResult.status = 1;
                    jsonResult.code = 0;
    ...
    private string GetExamUserList()
    {
        ...
        try
        {
            ...
            var tableName = " T_Exams exam LEFT JOIN T_ExamUser exUser ON exUser.ExamGuid=exam.ExamGuid LEFT JOIN T_Users TUSER ON TUSER.UserGuid=exUser.UserGuid";
