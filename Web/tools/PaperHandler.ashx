...
public class PaperHandler : IHttpHandler
{
    ...
    private string PaperOp()
    {
        ...
        try
        {
            ...
            if (bll.PaperOp(paperGuid, paperName, decimal.Parse(score), paperRemark, questionGuid))
            {
                jsonResult.status = 1;
                jsonResult.msg = "操作成功";
            }
    ...
    private string Delete()
    {
        ...
        try
        {
            var paperGuid = HttpContext.Current.Request.Params["paperGuid"] + "";
            var bll = new ExamBLL.ProcedureBLL();
            if (ExamDAL.DbHelperSQL.ExecuteSql("UPDATE T_Paper SET Status=0 WHERE PaperGuid=@PaperGuid", new SqlParameter("@PaperGuid", paperGuid)) > 0)
            {
                jsonResult.status = 1;
