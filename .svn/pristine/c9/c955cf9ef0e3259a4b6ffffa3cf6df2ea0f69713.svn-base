...
namespace ExamCommon
{
    public class Utils2
    {
        ...
        /// <param name="Sleep">是否要在生成前将当前线程阻止以避免重复</param>
        /// <returns></returns>
        public static string GetStringCode(int Length, bool Sleep)
        {
            if (Sleep)
                System.Threading.Thread.Sleep(3);
        ...
        public static bool IsUrl(string str)
        {
            ...
            string regEx = @"^((https?|ftp|news):\/\/)?([a-z]([a-z0-9\-]*[\.。])+([a-z]{2}|aero|arpa|biz|com|coop|edu|gov|info|int|jobs|mil|museum|name|nato|net|org|pro|travel)|(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(\/[a-z0-9_\-\.~]+)*(\/([a-z0-9_\-\.]*)(\?[a-z0-9+_\-\.%=&]*)?)?(#[a-z][a-z0-9_]*)?$";
        ...
        public static string ToPypellCode(string txt)
        {
            ...
            return result.ToString().Where(c => reg.IsMatch(c + "")).Aggregate("", (current, c) => current + c);
        }
        #endregion
