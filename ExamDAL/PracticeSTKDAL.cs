...
namespace ExamDAL
{
	...
	/// 数据访问类STKDAL。
	/// </summary>
	public class PracticeSTKDAL
	{
		public PracticeSTKDAL()
		{}
		#region  成员方法
		...
		public bool Exists(Guid STKGuid)
		{
			...
			strSql.Append("select count(1) from T_PracticeSTK");
		...
		public void Add(STKEntity model)
		{
			...
			strSql1.Append("insert into T_PracticeSTK(");
		...
		public void Update(STKEntity model)
		{
			StringBuilder strSql=new StringBuilder();
			List<SqlParameter> lst = new List<SqlParameter>();
			strSql.Append("update T_PracticeSTK set ");
			if (model.STKName != null)
			{
		...
		public void Delete(Guid STKGuid)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from T_PracticeSTK ");
			strSql.Append(" where STKGuid=@STKGuid ");
			SqlParameter[] parameters = {
					new SqlParameter("@STKGuid", SqlDbType.UniqueIdentifier)};
			parameters[0].Value = STKGuid;
		...
		public STKEntity GetModel(Guid STKGuid)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 STKGuid,STKName,CreateTime,Status from T_PracticeSTK ");
			strSql.Append(" where STKGuid=@STKGuid ");
			SqlParameter[] parameters = {
					new SqlParameter("@STKGuid", SqlDbType.UniqueIdentifier)};
			parameters[0].Value = STKGuid;
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM T_PracticeSTK ");
			if(strWhere.Trim()!="")
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 [STKID] ,[STKGuid] ,[STKName],[CreateTime] ,[Status] from T_PracticeSTK ");
            strSql.Append(" where  " + strWhere);
